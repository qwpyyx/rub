...
4. 如果是在 Linux/Mac 终端运行，可以直接设置以下环境变量：
   ```bash
   ...
   ```
5. 如果是双击 `main.exe` 直接打开，在浏览器访问 http://127.0.0.1:8080 后，先在页面顶部的「短信提醒配置」区域填写 AccessKeyId、AccessKeySecret、短信签名、模板编码并保存；信息会写入同目录下的 `sms_config.json`，后续再次打开 exe 会自动读取。
6. 确保用户信息中填有手机号（在 Web 表单或配置文件中添加），短信才会发送成功。